...
- **Release (signiert + notarisiert + DMG):** `bash wrappers/sign-and-release.sh` →
  ...
  Rezept: `projektextern: macos-app-distribution.md`. **Audio-Reaktivität** nutzt
...
- Spec: `design-proposal/THEME-PLAN.md`. **Material-Texturen per img2img auf dem Mac** (mflux/
...
Theme-Texturen wurden per **img2img auf dem Mac** (mflux) erzeugt. Während des ersten Laufs
...
- Falls ssh doch wieder bricht: auf dem Mac `ls -la ~/.ssh/authorized_keys`, `chmod 700 ~/.ssh;
